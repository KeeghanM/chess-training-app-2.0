...
export default function CalculationTrainer() {
  ...
  const goToNextPuzzle = async (status: string) => {
    ...
    if (status == 'correct') {
      await trackEventOnClient('calculation_correct', {})
      await fetch('/api/calculation/streak', {
        method: 'POST',
        body: JSON.stringify({ currentStreak: currentStreak + 1 }),
      })
      setCurrentStreak(currentStreak + 1)
    } else if (status == 'incorrect') {
      await trackEventOnClient('calculation_incorrect', {})
    }
  ...
  const markMoveAs = async (status: 'correct' | 'incorrect') => {
    ...
    await trackEventOnClient(`calculation_${status}`, {})
    ...
    <>
      <div className="flex flex-col gap-4 bg-purple-700 p-4">
        ...
        <Button
          ...
          onClick={async () => {
            setMode('training')
            await trackEventOnClient('calculation_start', {})
          }}
        >
          Start Training
        </Button>
