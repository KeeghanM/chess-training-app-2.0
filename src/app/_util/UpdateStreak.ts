...
export async function UpdateStreak(userId: string) {
  ...
  try {
    ...
    const todayString = now.toISOString().split('T')![0] as string
    const lastTrainedString = lastTrained.toISOString().split('T')![0] as string

    if (todayString != lastTrainedString) {
      const yesterdayString = yesterday.toISOString().split('T')![0] as string
